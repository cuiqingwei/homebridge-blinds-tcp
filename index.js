...
module.exports = function (homebridge) {
    ...
    homebridge.registerAccessory("homebridge-blinds-tcp", "BlindsTCP", BlindsTCPAccessory);
};

function BlindsTCPAccessory(log, config) {
    // global vars
    this.log = log;
...
BlindsTCPAccessory.prototype.getCurrentPosition = function(callback) {
    this.log("Requested CurrentPosition: %s", this.lastPosition);
    callback(null, this.lastPosition);
}

BlindsTCPAccessory.prototype.getPositionState = function(callback) {
    this.log("Requested PositionState: %s", this.currentPositionState);
    callback(null, this.currentPositionState);
}

BlindsTCPAccessory.prototype.getTargetPosition = function(callback) {
    this.log("Requested TargetPosition: %s", this.currentTargetPosition);
    callback(null, this.currentTargetPosition);
}

BlindsTCPAccessory.prototype.setTargetPosition = function(pos, callback) {
    ...
    this.tcpRequest(this.host, this.port, (moveUp ? this.upPayload : this.downPayload), function() {
    ...
    this.interval = setInterval(function(){
      ...
      if (localThis.lastPosition == localThis.currentTargetPosition) {
        if (localThis.currentTargetPosition != 0 && localThis.currentTargetPosition != 100) {
          localThis.tcpRequest(localThis.host, localThis.port, localThis.stopPayload, function() {
...
BlindsTCPAccessory.prototype.tcpRequest = function(host, port, payload, callback) {
        tcp(host, port, payload, function (err) {
            callback(err);
        });
    },

BlindsTCPAccessory.prototype.getServices = function() {
  return [this.service];
}
